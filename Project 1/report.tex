...
required for larger systems. This is usually given in $\bigO{n}$ notation.
...
For a general matrix with size ($N\times N$),
the first operation is subtraction the first row times a constant from
all the remaining rows. This is $\bigO{n^2}$ operations. This is done for all 
N columns, which gives a complexity of $\bigO{n^3}$. The backwards 
substitution is only $\bigO{n^2}$ and is not counted.
...
This method uses the Gaussian decomposition and should be solving
the equations in $\bigO{n^3}$.
