...
and the energies,
\textcolor{red}{MU: I will fix this}
\begin{gather*}
States = 
\begin{cases}
...
\textcolor{red}{MU: I'm fixing this}
\begin{gather}
Z = \sum_i d(i) e^{-E_i\beta} = 2e^{+16J}+12e^{0}+2e^{-16J}
 = 12 + 4\cosh(16J\beta)
\end{gather}
\begin{gather}
\expval{E} = 
...
\begin{algorithm}
 1) Pick a random atom, $s_0$ by drawing two random integers, $x$ and $y$ in the lattice domain.
 
 2) Suggest a spin-flip and compute possible change in energy $\Delta E$.
\begin{align*} 
\Delta E = -2s_0(s_1+s_2+s_3+s_4), \quad \Delta E \in \{-8,-4,0,4,8 \} 
\end{align*} 


 3) \textsc{If} $\Delta E \leq 0 \quad \to$ Accept spin-flip. \\
\textsc{Else} Draw random number, $r \in [0,1]$ \\
\textsc{	If} $r \leq exp(-\Delta E \beta ) \quad \to$ Accept spin-flip. \\
\textsc{Else} Reject flip. 
