...
Using a forward discretization of the time-derivative we obtain the following finite difference approximation of the diffusion equation:
\begin{align}
...
Using a backward discretization of the time-derivative we obtain the following finite difference approximation of (2.11):
\begin{align}
...
\item Get vector u, and the numbers a and b
\item Loop over all but the ends,
\begin{gather*}
d_i = bu_{i-1} + au_i + bu_{i+1}
\end{gather*}
\item Compute the ends, 
\begin{align*}
 d_1 &= ad_0 + bd_1\\
...
\end{enumerate}
\end{algorithm}
The inverse matrix multiplication uses the same method as in project 1, 
which is in the Github repository.

\subsection{Difference schemes}
