...
\subsubsection{Explicit Euler}
The forward time-derivative is obtained by solving (3.11) for $u'(a)$, with $a=t$ and $h=\Delta t$
\begin{equation}
\frac{u(t+\Delta t)-u(t)}{\Delta t} = u'(t)+O(\Delta t).
\end{equation}  
Having to divide by $h$ we get an truncation error of $O(h)$ in time.

The second derivative with respect to space is obtained by adding (3.12), with $a=x$,  $h=\Delta x$ and 
...
instead of $h=\Delta t$ and hence recover the same truncation error, $O(\Delta x^2)+O(\Delta t)$. However, it is "cheaper", computational wise, to obtain more accuracy using this scheme for stability reasons(discussed in section (3.3.2)). 

\subsubsection{Crank-Nicholson}
To ensure better accuracy a center derivative in time can be used as well. By letting $a=t+\Delta t/2$, $h=\Delta t$ and subtracting $a=t-\Delta t/2$, $h=-\Delta t$ for the series (3.13) we get the following center time-derivative:
\begin{equation}
...
\begin{table}[H]
\centering
  \caption{Data from numerical experiments. Computing error according to (4.1) at T=0.01.}
\vspace{2mm}
...
\begin{tabular}{ |l|l|l|l|l|l| }
  ...
  \hline
1/10	 & 0.0615769 & 	0.0646733 & 	0.0466711 & 	0.0650734 & 	0.023262  \\
\hline
1/100 &	0.0121561 & 	0.010757 & 	0.011458 & 	0.0163719 & 	0.294685  \\
\hline
\end{tabular}
\vspace{0.2cm}
\end{table}
...
\begin{table}[H]
\centering
  \caption{Data from numerical experiments. Computing error according to (4.1) at T=1.0.}
\vspace{2mm}
...
\begin{tabular}{ |l|l|l|l|l|l| }
  ...
  $\Delta x$ & \textbf{FE} & \textbf{BE} & \textbf{CN}  & \textbf{MC1} & \textbf{MC2}  \\
  \hline
1/10 &	0.015625 &	1.43456667265 & 1  & 1 & 2  \\
\hline
1/100 &	0.015625 &	1.43456667265 & 1  & 1 & 2  \\
\hline
\end{tabular}
\vspace{0.2cm}
\end{table}

\subsection{Example of run}
...
The result for these parameters give figure \ref{fig:dx_vs_t}. 
In figure \ref{subfig:smallxbigt} we see that all methods except the 
Monte Carlo with 
variable time-step reaches the steady state, and is thus
overlaying each other.
The Monte Carlo simulation struggles with the normalisation, and so the 
first bin is overfull. This causes trouble with the rate, and so is not 
a reliable method as implemented here. For shorter times, figure 
\ref{subfig:smallxsmallt}, there is a 
discrepancy between all the different methods, with the Monte Carlo with 
variable time-step following the analytical solution the closest.

With a finer division of the axis, we obtain figure \ref{subfig:smallxbigt}.
This shows that the matrix solvers and Monte Carlo method with equal 
step-length all reach the same solution as the analytical. The 
Monte Carlo with unequal steps suffer yet again from the normalisation.
...
data folder under report. This should be necessary information to 
rerun all the results in this report.


\section{Conclusion}

From the data gathered we have found a good way to implement all the 
differing methods, apart from the Monte Carlo method with variable 
step-length. For this method some further tweaking to the normalisation 
is required in order for the method to be accurate. It does show some 
promising features, especially for short pulses, but suffers greatly 
when they start to flow to the left.
